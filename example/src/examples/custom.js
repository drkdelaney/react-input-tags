...
const debug = () => () => {}; // Add debugging component on ui
let idTracker = 2;

export default class Simple extends Component {
    state = {
        tags: [
            { id: 0, text: 'Last, First 1' },
            { id: 1, text: 'Last, First 2' },
        ],
    };
    ...
    handleNewTag = value => {
        if (value)
            this.setState(({ tags }) => ({
                tags: tags.concat({ id: idTracker++, text: value }),
            })); //eslint-disable-line
        debug('handleInputEnter')(
            JSON.stringify({ id: idTracker++, text: value })
        ); //eslint-disable-line
    };
        ...
        return (
            <div>
                <InputTags
                    ...
                    tagContainerClass="newContainer"
                    tagClass="newTag"
                    closeClass="newClose"
                    tagInputClass="newInput"
                />
            </div>
        );
    }
